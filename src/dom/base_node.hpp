...
class BaseNode {
 public:
  ...
  virtual const std::vector<std::shared_ptr<BaseNode>>& Children() const noexcept = 0;
  virtual const std::unordered_map<std::string, std::string>& Attributes() const noexcept = 0;
  virtual const std::vector<std::string>& Classes() const noexcept = 0;

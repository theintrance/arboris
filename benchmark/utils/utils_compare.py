...
# lxml: XPath 기반 질의
def lxml_parse_and_query(html_bytes: bytes) -> dict:
  ...
  a_cnt = len(doc.xpath("//a[@href]"))
  img_cnt = len(doc.xpath("//img[@src]"))

  # 스크립트/스타일 제외 + 공백 정규화
  texts = doc.xpath("//text()[normalize-space() and not(ancestor::script) and not(ancestor::style)]")
...
def selectolax_parse_and_query(html_bytes: bytes) -> dict:
  ...
  body_text = body.text() if (body and hasattr(body, "text") and body.text) else ""
  body_text = " ".join(body_text.split())

  return {
    "title_len": len(title.text()) if title else 0,
    "meta_desc_len": len(meta_desc.attributes.get("content","")) if meta_desc else 0,
    "og_count": len(ogs),
    "a_img_count": a_cnt + img_cnt,

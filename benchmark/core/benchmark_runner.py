...
class BenchmarkRunner:
    ...
    def run_benchmark(self, parser: BaseParser, document_type: str = "html") -> BenchmarkResult:
        ...
        if parse_times:
            ...
            throughput_docs_per_sec = (
                successful_parses / (total_time / 1000) if total_time > 0 else 0
            )
            throughput_mb_per_sec = (
                (total_bytes / (1024 * 1024)) / (total_time / 1000) if total_time > 0 else 0
            )
    ...
    def print_results(self, results: list[BenchmarkResult] | None = None):
        ...
        for result in results:
            ...
            print(
                f"  파일 수: {result.total_files} (성공: {result.successful_parses}, 실패: {result.failed_parses})"
            )
            ...
            print(
                f"  처리량: {result.throughput_docs_per_sec:.2f} docs/sec, {result.throughput_mb_per_sec:.2f} MB/sec"
            )
